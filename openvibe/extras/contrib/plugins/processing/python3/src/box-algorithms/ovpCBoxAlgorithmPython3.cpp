...
static bool setMatrixInfosFromPyObject(PyObject* obj, CMatrix* matrix)
{
	...
	for (size_t i = 0; i < nDim; ++i)
	{
		...
		for (size_t j = 0; j < size; ++j)
		{
			...
			if (PyUnicode_Check(label)) { matrix->setDimensionLabel(i, j, PyUnicode_AsUTF8(label)); }
			else
			{
				Py_CLEAR(pySizeDim);
				Py_CLEAR(pyLabelDim);
...
bool CBoxAlgorithmPython3::initializePythonSafely()
{
	...
	this->getLogManager() << Kernel::LogLevel_Info << "file open check\n";
	...
	this->getLogManager() << Kernel::LogLevel_Info << "file run check\n";
	*/
	const std::string path = std::string(Directories::getDataDir().toASCIIString()) + "/plugins/python3/openvibe.py";
	std::ifstream file;
	file.open(path);
	if (!file.is_open())
	{
...
bool CBoxAlgorithmPython3::initialize()
{
	...
	for (size_t i = 0; i < boxCtx.getInputCount(); ++i)
	{
		...
		if (typeID == OV_TypeId_StreamedMatrix) { m_decoders.push_back(new Toolkit::TStreamedMatrixDecoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_Signal) { m_decoders.push_back(new Toolkit::TSignalDecoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_FeatureVector) { m_decoders.push_back(new Toolkit::TFeatureVectorDecoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_Spectrum) { m_decoders.push_back(new Toolkit::TSpectrumDecoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_ChannelLocalisation) { m_decoders.push_back(new Toolkit::TChannelLocalisationDecoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_Stimulations) { m_decoders.push_back(new Toolkit::TStimulationDecoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_ExperimentInfo) { m_decoders.push_back(new Toolkit::TExperimentInfoDecoder<CBoxAlgorithmPython3>(*this, i)); }
		else
		{
	...
	for (size_t i = 0; i < boxCtx.getOutputCount(); ++i)
	{
		...
		if (typeID == OV_TypeId_StreamedMatrix) { m_encoders.push_back(new Toolkit::TStreamedMatrixEncoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_Signal) { m_encoders.push_back(new Toolkit::TSignalEncoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_FeatureVector) { m_encoders.push_back(new Toolkit::TFeatureVectorEncoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_Spectrum) { m_encoders.push_back(new Toolkit::TSpectrumEncoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_ChannelLocalisation) { m_encoders.push_back(new Toolkit::TChannelLocalisationEncoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_Stimulations) { m_encoders.push_back(new Toolkit::TStimulationEncoder<CBoxAlgorithmPython3>(*this, i)); }
		else if (typeID == OV_TypeId_ExperimentInfo) { m_encoders.push_back(new Toolkit::TExperimentInfoEncoder<CBoxAlgorithmPython3>(*this, i)); }
		else
		{
	...
	if (pyTmp == nullptr) { this->getLogManager() << Kernel::LogLevel_Error << "Failed to load [" << m_scriptFilename << "]"; }

	PyObject* result = PyObject_CallObject(m_execFileFunction, pyTmp);
	if (result == nullptr || PyLong_AsLong(result) != 0)
	{
		this->getLogManager() << Kernel::LogLevel_Error << "Failed to run [" << m_scriptFilename << "], result = "
	...
	if (m_boxInitialize && PyCallable_Check(m_boxInitialize))
	{
		...
		{
			if (res == nullptr) { this->getLogManager() << Kernel::LogLevel_Error << "Failed to call \"box.__initialize\" function.\n"; }
...
bool CBoxAlgorithmPython3::transferStreamedMatrixInputChunksToPython(const size_t index)
{
	...
	for (size_t idx = 0; idx < boxCtx.getInputChunkCount(index); ++idx)
	{
		...
		{
			CMatrix* matrix = dynamic_cast<Toolkit::TStreamedMatrixDecoder<CBoxAlgorithmPython3>*>(m_decoders[index])->getOutputMatrix();
...
{
	Kernel::IBoxIO& boxCtx = this->getDynamicBoxContext();
	CMatrix* matrix        = dynamic_cast<Toolkit::TStreamedMatrixEncoder<CBoxAlgorithmPython3>*>(m_encoders[index])->getInputMatrix();
	...
	for (size_t idx = 0; idx < len; ++idx)
	{
		...
		if (PyObject_IsInstance(pyChunk, m_matrixHeader) == 1)
		{
			...
			{
				this->getLogManager() << Kernel::LogLevel_Error << "Failed setting matrix info.\n";
				Py_CLEAR(pyChunk);
				return false;
			}
...
bool CBoxAlgorithmPython3::transferSignalInputChunksToPython(const size_t index)
{
	...
	for (size_t idx = 0; idx < boxCtx.getInputChunkCount(index); ++idx)
	{
		...
		if (m_decoders[index]->isHeaderReceived())
		{
			CMatrix* matrix = dynamic_cast<Toolkit::TSignalDecoder<CBoxAlgorithmPython3>*>(m_decoders[index])->getOutputMatrix();
			size_t nDim     = matrix->getDimensionCount();
			...
			if (PyTuple_SetItem(
					pyArg, 4, PyLong_FromLong(long(dynamic_cast<Toolkit::TSignalDecoder<CBoxAlgorithmPython3>*>(m_decoders[index])->getOutputSamplingRate())))
				!= 0)
			{
...
{
	Kernel::IBoxIO& boxCtx = this->getDynamicBoxContext();
	CMatrix* matrix        = dynamic_cast<Toolkit::TSignalEncoder<CBoxAlgorithmPython3>*>(m_encoders[index])->getInputMatrix();
	...
	for (size_t idx = 0; idx < len; ++idx)
	{
		...
		if (PyObject_IsInstance(pyChunk, m_signalHeader) == 1)
		{
			...
			Kernel::TParameterHandler<uint64_t>& sampling = dynamic_cast<Toolkit::TSignalEncoder<CBoxAlgorithmPython3>*>(m_encoders[index])->
...
bool CBoxAlgorithmPython3::transferStimulationInputChunksToPython(const size_t index)
{
	...
	for (size_t idx = 0; idx < boxCtx.getInputChunkCount(index); ++idx)
	{
		...
		if (m_decoders[index]->isBufferReceived())
		{
			CStimulationSet* stimSet = dynamic_cast<Toolkit::TStimulationDecoder<CBoxAlgorithmPython3>*>(m_decoders[index])->getOutputStimulationSet();
...
{
	Kernel::IBoxIO& boxCtx   = this->getDynamicBoxContext();
	CStimulationSet* stimSet = dynamic_cast<Toolkit::TStimulationEncoder<CBoxAlgorithmPython3>*>(m_encoders[index])->getInputStimulationSet();
...
#endif // else WIN32 && TARGET_BUILDTYPE_Debug
